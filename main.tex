...
\subsection*{Robustness of lipid changes across genotypes}                                         
                                                                                                     
To assess whether observed lipid differences between C and LI conditions reflected population-wide metabolic shifts rather than outlier-driven artifacts, we performed two-sample Wilcoxon rank-sum tests with leave-one-out jackknife stability analysis at both the class-ratio and individual species levels. At the class level, all 33 lipid ratios showed significant differences between conditions (Benjamini-Hochberg adjusted $p < 0.05$), with each achieving perfect jackknife stability (1.0; Supplementary Table~S7). The largest effects were observed for phosphatidylserine-related ratios (e.g., PS/SQDG fold-change = 12.78). At the individual species level, 134 of 218 lipids were significantly altered, and 139 species achieved perfect jackknife stability (1.0; Supplementary Table~S7b). Across all 13 lipid classes, the minimum species-level jackknife stability was 1.0 (Supplementary Table~S7c), confirming that even species-specific changes were not driven by extreme genotypes. These results indicate that the observed lipidomic remodeling under low-input conditions represents a consistent, population-wide response across the SAP genotypes tested.    

\subsection*{Lipidome overview under Control and Low-input field conditions}

Across both environments, we quantified 244 distinct lipid molecular species, with substantial overlap between conditions (153 shared species) and smaller environment-specific subsets (49 Control-exclusive; 42 LowInput-exclusive; Supplementary Fig.~S3A; Supplementary Table~S3). TG and PC represented the most species-rich classes in each environment (TG: 71 in Control, 69 in LowInput; PC: 39 in Control, 38 in LowInput; Supplementary Table~S3), and broader class-level summaries indicated that glycerolipids and glycerophospholipids constituted the majority of identified species (Supplementary Fig.~S3B,D). PCA of lipid species further revealed that the multivariate structure of the dataset was strongly driven by lipid class, with TGs separating from membrane lipid classes and galactolipids forming distinct clusters under both environmental conditions (Supplementary Fig.~S4).

At the lipid class level, the lipidome was predominantly composed of galactolipids and  phospholipids, enriched with DG and TG (Supplementary Fig.~S2A). Under C conditions, MGDG accounted for 34.5\% of the mean TIC and declined to 32.1\% under LI, whereas PC remained comparatively stable (25.6\% in C versus 24.8\% in LI). In contrast, TG increased from 2.7\% (C) to 5.4\% (LI), and SQDG decreased from 2.9\% (C) to 1.4\% (LI) (Supplementary Fig.~S2A). Compositional contrast analyses corroborated these trends and further revealed a coordinated headgroup rebalancing within the phospholipid network. CLR contrasts also highlighted PS as one of the strongest positively shifted classes (Fig.~\ref{fig:fig1_lipidomics_landscape}B; Supplementary Table~S2). Consistently, TG-referenced ALR contrasts and ratio-based comparisons showed that PS increased relative to multiple phospholipid pools (e.g., PC, PE, and PG), even when absolute \%TIC changes in those classes were modest (Supplementary Fig.~S2B). Moreover, lysophospholipids exhibited a reproducible compositional remodeling signal, with LPC/LPE behavior shifting under LI in a manner consistent with stress-responsive phospholipid turnover rather than uniform scaling of total lyso-lipid abundance (Supplementary Fig.~S2B; Fig.~2 and Supplementary Fig.~S9) (ref). Together, these results support four LI-associated lipid signatures: (i) SQDG depletion, (ii) TG enrichment, (iii) PS enrichment relative to other phospholipid headgroups, and (iv) lysophospholipid remodeling (LPC/LPE), consistent with coordinated membrane restructuring and increased partitioning into storage lipids.
...
Finally, given that the SAP encompasses broad genetic diversity, we next evaluated lipid variance across genotypes and identified MGDG(18:3/18:3) as the highest-variance lipid in both environments (Supplementary Fig.~S5A,B; Supplementary Table~S5). Seven of the ten highest-variance lipids were shared between C and LI (Supplementary Fig.~S5C), suggesting that a conserved subset of lipid traits exhibits strong genotype sensitivity across environments. Within this high-variance subset, PC species were the most frequent class in both conditions (C: 7/10; LI: 6/10; Supplementary Fig.~S5D; Supplementary Table~S5), supporting their prioritization for subsequent genetic mapping and predictive modeling analyses.
...
% =========================
% SUPPLEMENTARY TABLES (S2, S3, S5) for Lipidome Overview
% Note: Supplementary figures S1-S5 are defined in the Supplement section at the end
% =========================

\begin{table*}[p]
\centering
\caption{\textbf{Supplementary Table S2A: Class-level CLR (centered log-ratio) contrasts between LowInput and Control.} Positive effects indicate enrichment under LowInput; negative effects indicate depletion. CI = 95\% confidence interval.}
\label{tab:supptable_s2a}
\small
\begin{tabular}{lrrr}
\toprule
Class & Effect & CI Low & CI High \\
\midrule
PS    &  0.98 &  0.93 &  1.03 \\
LPE   &  0.57 &  0.54 &  0.60 \\
TG    &  0.47 &  0.43 &  0.50 \\
...
\begin{table*}[p]
\centering
\caption{\textbf{Supplementary Table S3: Summary of lipid species counts and overlap between Control and LowInput conditions.}}
\label{tab:supptable_s3}
\small
\begin{tabular}{lr}
\toprule
Metric & Count \\
...
\begin{table*}[p]
\centering
\caption{\textbf{Supplementary Table S5: Top 10 high-variance lipids by condition.} Lipids ranked by genotypic variance across the SAP.}
\label{tab:supptable_s5}
\small
\begin{tabular}{clllr}
\toprule
Rank & Condition & Lipid & Class & Variance \\
\midrule
1 & Control & MGDG(18:3/18:3) & MGDG & 8.58 \\
2 & Control & PC(16:0/18:2) & PC & 2.91 \\
3 & Control & DG(18:3/18:3) & DG & 1.01 \\
4 & Control & PC(16:0/18:1) & PC & 0.92 \\
5 & Control & PC(16:0/18:0) & PC & 0.37 \\
6 & Control & DGDG(18:2/18:4) & DGDG & 0.32 \\
7 & Control & PC(16:0/18:3) & PC & 0.31 \\
8 & Control & PC(18:1/16:0) & PC & 0.24 \\
9 & Control & PC(16:0/20:5) & PC & 0.22 \\
10 & Control & PC(16:0/20:4) & PC & 0.16 \\
\midrule
1 & LowInput & MGDG(18:3/18:3) & MGDG & 4.57 \\
2 & LowInput & PC(16:0/18:2) & PC & 1.90 \\
3 & LowInput & PC(16:0/18:0) & PC & 0.95 \\
4 & LowInput & DG(18:3/18:3) & DG & 0.71 \\
5 & LowInput & DGDG(18:2/18:4) & DGDG & 0.46 \\
6 & LowInput & PC(16:1/18:2) & PC & 0.33 \\
7 & LowInput & PC(16:0/18:3) & PC & 0.27 \\
8 & LowInput & PC(16:0/20:5) & PC & 0.22 \\
9 & LowInput & PC(18:0/18:1) & PC & 0.18 \\
10 & LowInput & TG(18:2/18:2/22:1) & TG & 0.14 \\
\bottomrule
\end{tabular}
\end{table*}
...
% =========================
% MAIN FIGURE 2 - OPLS-DA
% =========================

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/main/Figure2_OPLS_DA.png}
  \caption{\textbf{OPLS-DA of lipid class ratios discriminates Control from LowInput.}
  (A) Score plot showing separation along the predictive component.
  (B) VIP-ranked lipid class ratios (VIP $>$ 1).
  (C) Permutation test (n=200) validating model performance.
  (D) Model diagnostics ($Q^2$, $R^2Y$, $R^2X$).}
  \label{fig:fig2_opls_da}
\end{figure*}

\begin{figure*}[p]
  \centering
  \includegraphics[width=\textwidth]{figures/supp/SuppFig_S6_Partial_Correlations.png}
  \caption{\textbf{Partial correlations among lipid classes in Control and LowInput conditions.} Sign reversals indicate altered coordination among lipid pools under stress.}
  \label{fig:suppfig_s6_partial_corr}
\end{figure*}

\begin{figure*}[p]
  \centering
  \includegraphics[width=\textwidth]{figures/supp/SuppFig_S7_Lipid_Ratios.png}
  \caption{\textbf{Lipid class ratios discriminating Control and LowInput.} (A) VIP-ranked ratios from OPLS-DA. (B) Ratios derived from partial correlation sign-flip relationships.}
...
10^{-9}$, 4–5 traits), notably, carries a phosphorus-responsive functional annotation, suggesting possible cross-talk between cold and phosphate signaling pathways.                                                                        
  ...
  & SORBI\_3002G128400 / WRKY$^{*}$ & $3.6 \times 10^{-9}$ & 4 & $3.6 \times 10^{-9}$ & 5 \\         
  \bottomrule                                                                                        
  \end{tabular}                                                                                      
  \begin{flushleft}                                                                                  
  \scriptsize{$^{*}$Also annotated as phosphorus-responsive. --: not significant at $p \le 10^{-7}$.}
  \end{flushleft}                                                                                    
  \end{table}           
...
\begin{figure*}[p]
  \centering
  \includegraphics[width=\textwidth]{SuppFig_S8_GWAS.png}
  \caption{\textbf{Supplementary GWAS Manhattan plots for Control condition.}}
  \label{fig:suppfig_s8_gwas}
\end{figure*}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Figure3_GWAS.png}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{Figure5_LINEX_Enriched_Subnetwork.png}
  \caption{\textbf{Enriched lipid reaction network from LINEX analysis.}
  Lipid species (circles) are connected to enzymes (squares) catalyzing reversible reactions. Node colors indicate lipid class (purple: MG/DG; orange: TG; green shades: phospholipids and lysophospholipids). Arrows indicate reaction direction. LCAT* (italic) denotes a hypothesized enzyme not annotated in LINEX but inferred from reaction specificity.}
  \label{fig:fig5_linex}
\end{figure*}


\subsection*{Lipid modules predict plant height with high accuracy and cross-dataset reproducibility}

To identify lipids predictive of agronomic traits, we applied XGBoost regression with SHAP-based interpretation. Given the high correlation among lipid species within biochemical classes, we first grouped the 226 individual lipids into 27 correlation-derived modules using hierarchical clustering based on Spearman correlation distances (Methods). This module-level strategy mitigates the  limitation whereby SHAP importance values become attenuated and unstable in the presence of highly collinear predictors (ref).

\paragraph{Plant height prediction.}
For plant height, the XGBoost model exhibited robust predictive performance on our dataset ($R^2 = 0.752 \pm 0.027$, 5-fold CV; $n = 388$; Fig.~4A). Module-level SHAP analysis indicated that a single TG-dominated module (M1; 22 lipids) contributed the majority of the predictive signal, with a mean absolute module SHAP approximately 10-fold higher than the second-ranked module (Fig.~4B). Lipids assigned to M1 consisted primarily of short- and medium-chain TG, including TG(10:0/10:0/10:0), TG(12:0/12:0/14:0), and TG(10:0/12:0/16:0). 
...
\subsection{Lipid GWAS Identifies a Phosphate Starvation Response Gene}
...
\begin{figure*}[!ht]
  ...
  (A,B) Run-order TIC plots for Control and LowInput conditions showing stable analytical performance.
  (C,D) QC-RSD distributions before and after SERRF normalization.
  (E,F) PCA score plots before and after normalization showing tighter sample clustering.
  (G,H) SpATS spatial residual maps indicating mitigation of field-position effects.}
...
\begin{figure*}[!ht]
  ...
  (A) Lipid class composition (percent TIC) in Control and LowInput conditions.
  (B) TG-referenced ALR (additive log-ratio) contrasts showing relative shifts across lipid classes.}
...
\begin{figure*}[!ht]
  ...
  (A) Venn diagram of species overlap between Control and LowInput.
  (B) Species counts by lipid class in each condition.
  (C) Euler diagram of shared vs.\ exclusive species.
  (D) Superclass-level summary of identified lipid species.}
...
\begin{figure*}[!ht]
  ...
  Principal component analysis of lipid species reveals strong separation by lipid class, with TG species clustering distinctly from membrane phospholipids and galactolipids.}
...
\begin{figure*}[!ht]
  ...
  (A,B) Top 10 highest-variance lipids ranked by genotypic variance in Control and LowInput.
  (C) Venn diagram showing overlap of top-variance lipids between conditions.
  (D) Class distribution of top-variance lipids.}
