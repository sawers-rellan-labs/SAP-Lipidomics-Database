...
We performed GWAS using three representations of the same lipidomics dataset: (i) individual lipid species, (ii) summed abundances at the level of lipid classes, and (iii) all possible pairwise ratios among these aggregated classes. Statistical significance was defined as $p \le 10^{-7}$ (equivalently, $-\log_{10}(p) \ge 7$), and SNPs were mapped to genes using a symmetric 25~kb flanking window.

Using this stringent threshold, we identified 11,286 unique genes associated with at least one individual lipid species and 15,975 genes associated with at least one summed-class or ratio phenotype under LowInput conditions. The most highly connected gene in the individual-lipid GWAS was \texttt{SORBI\_3010G064200} (chromosome 10), which exhibited significant associations with 20 distinct lipid phenotypes (best $p = 3.4 \times 10^{-16}$), predominantly short- and medium-chain triglycerides including TG(10:0\_16:0\_16:0), TG(12:0\_14:0\_16:0), and TG(14:0\_16:0\_18:1) (Table~\ref{tab:LI_individual_gene_top}). The second most-connected gene, \texttt{SORBI\_3001G057100} (chromosome 1), was associated with 16 lipid phenotypes spanning multiple classes including PC, DG, and TG species ($p_{\min} = 7.0 \times 10^{-67}$).

For the summed-class and ratio GWAS, the most highly connected gene was \texttt{SORBI\_3008G088836} (chromosome 8), which was associated with 44 distinct sum/ratio phenotypes (best $p = 2.1 \times 10^{-22}$), including numerous AEG-referenced ratios (Sum\_AEG/Sum\_Cer, Sum\_AEG/Sum\_DG, Sum\_AEG/Sum\_TG) as well as ceramide- and DG-referenced ratios (Table~\ref{tab:LI_sumratio_gene_top}). The second and third most-connected genes, \texttt{SORBI\_3006G093600} and \texttt{SORBI\_3004G127066}, were associated with 37 and 36 sum/ratio phenotypes, respectively, with minimum p-values reaching $10^{-14}$ to $10^{-125}$.

To identify and contextualize candidate genes, we used four complementary prioritization views: (1) genes with the highest occurrence across \emph{individual-lipid} GWAS phenotypes (Table~\ref{tab:LI_individual_gene_top}); (2) genes with the highest occurrence across \emph{summed-class and ratio} GWAS phenotypes (Table~\ref{tab:LI_sumratio_gene_top}); (3) lipid-class--focused GWAS (e.g., phospholipids and triacylglycerols) to prioritize class-relevant regulators; and (4) targeted follow-up of specific traits of interest, including sums (e.g., Sum~SQDG), biologically motivated ratios (cold-, nitrogen-, and phosphorus-linked), and individual compounds of interest (e.g., $\alpha$-carotene and zeaxanthin). We report LowInput results in the main text; full GWAS results and annotations for all traits are available in Supplementary Tables~S7--S9 and in the Shiny application.
...
\midrule
\texttt{SORBI\_3008G088836} & 44 & \texttt{SNP\_21312400} & $2.1\times10^{-22}$  & 14 & Sum\_\allowbreakAEG/Sum\_\allowbreakCer; Sum\_\allowbreakAEG/Sum\_\allowbreakDG; Sum\_\allowbreakAEG/Sum\_\allowbreakGalCer; Sum\_\allowbreakAEG/Sum\_\allowbreakSM; \ldots (44 total) \\
\texttt{SORBI\_3006G093600} & 37 & \texttt{SNP\_46347720} & $2.2\times10^{-14}$  & 11 & Sum\_\allowbreakAEG/Sum\_\allowbreakFA; Sum\_\allowbreakAEG/Sum\_\allowbreakPG; Sum\_\allowbreakAEG/Sum\_\allowbreakPS; Sum\_\allowbreakAEG/Sum\_\allowbreakTG; \ldots (37 total) \\
\texttt{SORBI\_3004G127066} & 36 & \texttt{SNP\_15426192} & $7.7\times10^{-125}$ & 17 & Sum\_\allowbreakAEG/Sum\_\allowbreakCer; Sum\_\allowbreakAEG/Sum\_\allowbreakDGDG; Sum\_\allowbreakAEG/Sum\_\allowbreakLPC; Sum\_\allowbreakAEG/Sum\_\allowbreakLPE; \ldots (36 total) \\
\texttt{SORBI\_3004G120000} & 35 & \texttt{SNP\_12933658} & $2.7\times10^{-115}$ & 16 & Sum\_\allowbreakAEG/Sum\_\allowbreakMG; Sum\_\allowbreakAEG/Sum\_\allowbreakMGDG; Sum\_\allowbreakAEG/Sum\_\allowbreakTG; Sum\_\allowbreakCer; \ldots (35 total) \\
\texttt{SORBI\_3009G090800} & 33 & \texttt{SNP\_17785435} & $1.1\times10^{-301}$ & 15 & Sum\_\allowbreakAEG/Sum\_\allowbreakDGDG; Sum\_\allowbreakAEG/Sum\_\allowbreakTG; Sum\_\allowbreakCer/Sum\_\allowbreakDGDG; Sum\_\allowbreakCer/Sum\_\allowbreakPA; \ldots (33 total) \\
\texttt{SORBI\_3009G086700} & 31 & \texttt{SNP\_14705908} & $4.2\times10^{-275}$ & 13 & Sum\_\allowbreakAEG/Sum\_\allowbreakDG; Sum\_\allowbreakAEG/Sum\_\allowbreakMG; Sum\_\allowbreakAEG/Sum\_\allowbreakMGDG; Sum\_\allowbreakAEG/Sum\_\allowbreakPA; \ldots (31 total) \\
\bottomrule
\end{tabularx}
\end{table}
...
To place the observed lipid species within a biochemical framework, we queried the LINEX2 (Lipid Network Explorer) database to identify enzyme-catalyzed reactions linking those lipids. Our analysis specifically focused on reactions involving glycerolipid and phospholipid classes. The resulting enriched subnetwork (Fig.~\ref{fig:fig5_linex}) comprises four enzymatic activities that define the interconversion among these lipid pools:

\begin{enumerate}                                                                                  
\item \textbf{LCAT-like transacylation (hypothesized):} Lecithin-cholesterol acyltransferase (LCAT)
catalyzes the reversible transfer of a fatty acyl chain from phosphatidylcholine to               
diacylglycerol, yielding lysophosphatidylcholine and triacylglycerol (PC + DG $\leftrightarrow$ LPC
+ TG). While canonical LCAT activity is best characterized in mammalian lipoprotein metabolism,   
plant homologs with analogous phospholipid:diacylglycerol acyltransferase (PDAT) activity have been
reported. We include this reaction as a hypothesized route consistent with the correlated shifts  
in PC/LPC and DG/TG ratios observed under LowInput.                                                
                                                                                                 
\item \textbf{PNPLA1-mediated TG hydrolysis:} Patatin-like phospholipase domain-containing protein 1 (PNPLA1) catalyzes the hydrolysis of TG to DG with the release of a free fatty acid (TG $\rightarrow$ DG + FA). This lipase activity provides a direct route for TG mobilization and DG regeneration.                                           

\item \textbf{LRO1 transacylation:} The phospholipid:diacylglycerol acyltransferase LRO1 transfers a fatty acyl chain from PE to DG, producing triacylglycerol and  LPE  (PE + DG $\rightarrow$ TG + LPE). This reaction couples phospholipid  turnover directly to TG biosynthesis, bypassing the canonical Kennedy pathway. 

\item \textbf{PNPLA3-mediated transacylation:} PNPLA3 catalyzes the CoA-independent transfer of fatty acids between DG and MG to form TG (DG + MG $\rightarrow$ TG). This transacylase activity enables TG synthesis from partial glycerides without requiring acyl-CoA substrates.
\end{enumerate}                                                                                    
                                                                                                 
A unifying feature of this subnetwork is the central position of diacylglycerol (DG), which participates as either substrate or product in all four reactions. This topology is consistent with DG functioning as a metabolic branch point that integrates signals from phospholipid remodeling (via LCAT and LRO1), storage lipid mobilization (via PNPLA1), and glyceride interconversion (via PNPLA3). The ratio-based multivariate analyses presented above identified DG-to-TG flux as a dominant axis of lipid remodeling under LowInput stress; the LINEX subnetwork provides candidate enzymatic routes through which this flux may be regulated.

Notably, lysophospholipid accumulation (LPC, LPE) emerges as a necessary byproduct of the LCAT and LRO1 reactions. Lysophospholipids are bioactive signaling molecules implicated in membrane stress responses, suggesting that the observed lipid remodeling may serve dual roles in both energy storage (TG accumulation) and stress signaling (lysophospholipid production). The full reaction annotations, including RHEA database identifiers and literature sources, are provided in Supplementary Table~S12.
...
\subsubsection*{Ratio- and ontology-based evidence supports coordinated redistribution across lipid pools under Low-input}
